...
const HabitstatisticsCard = (props) => {
    ...
    if(cardType === 'done') {
        cardHeader = <span className={classes['card-header']}><TiTick /> Complete</span>;
    } else if (cardType === 'fail'){
        cardHeader = <span className={classes['card-header']}><RxCross2 /> Failed</span>;
    } else if (cardType === 'skip'){
        cardHeader = <span className={classes['card-header']}><BsArrowRight /> Skipped</span>;
